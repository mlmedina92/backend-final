...
router.get('/', async (req, res) => { // si llamo al slash views renderio formualrio
    ...
    const prods = await fetch('http://localhost:3000/api/products');
    const dataProds = await prods.json()
            res.render('productsList', { 'prods': dataProds.payload, 'isAdminRole': req.session.role == 'admin', 'userName': req.session.userName })//cdo estoy en / se va a renderizar el handlebars home
}
})
