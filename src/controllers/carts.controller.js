...
export const purchaseCart = async (req, res) => {
    try {
        const cartApi = await fetch(getBaseUrl(req) + `/api/carts/${req.session.cartId}`) // obtengo el carrito
        ...
        for (const item of dataCart.products) {
            const prod = await fetch(getBaseUrl(req) + `/api/products/${item.productId}`) // obtengo todos los datos de los prods del carrito
        ...
        if (items.length > 0) { // se puede finalizar compras
            ...
            sendSuccessEmail( // sin await, dado que no es necesario, puede terminar de mandar el email luego de responder el request
                req,
                {
